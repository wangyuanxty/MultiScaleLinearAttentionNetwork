...
Figure~\subref{fig:cost} contrasts the per-window cost of this
...
Figure~\ref{fig:sqx} details the per-layer stage-query exchange, and
Figure~\ref{fig:gdn2} details the internal recurrence of one GDN-2
block.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig_arch_ms}
\caption{The multi-scale GDN-2 encoder: three parallel branches of
two layers each (8\,KB fixed state per layer). The $W{=}64$ window
is pooled with stride 2/4/8 into 32/16/8 tokens, embedded to
$d{=}64$, linked per layer by the stage-query exchange, fused at the
last token (192 $\to$ 128 $\to$ GELU $\to$ 1) to predict
$\hat{c}_{t+1}$, which rolls forward.}
\label{fig:arch}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.60\textwidth]{figures/fig_sqx_exchange}
\caption{The per-layer stage-query cross-scale exchange: the coarse
branch's GDN state $S$ (2048 elements) is projected to a single
query ($d_k{=}32$) that sweeps the fine/mid branch $k/v$ streams
($d_{\text{model}}{=}64$) by single-query linear attention
($O(L)$, no gating), merging additively
($h_f \leftarrow h_f + c_f$).}
\label{fig:sqx}
\end{figure}
...
(Figure~\subref{fig:patch_pool} shows the real capacity series at each
...
branches without interaction degrade consistently (MAE $0.0076$ vs
$0.0066$ on CALCE), and that the stage-query exchange outperforms a
single-branch backbone ($0.0066$ vs $0.0070$).
...
\ref{sec:ablation}): a degradation-rate head driven by a measured
