...
result of Zhao et al.\ \cite{ref_zhou2025}
...
\paragraph{Robustness to training protocol.}
The 100-epoch constant-LR setting above is not chosen adversarially:
a checkpoint-continuation study on NASA (seeds 1--3; 100 $\to$ 200
$\to$ 300 $\to$ 500 epochs) shows that at every epoch level the
single branch remains at or near the best configuration (means
$0.0090$/$0.0079$/$0.0078$/$0.0081$), while the multi-scale
configs reach shallow optima near 300 epochs and never overtake it.
Both a cosine decay ($10^{-3}\to10^{-6}$) and a constant $10^{-4}$
learning rate strictly hurt every configuration, and selecting each
run's personally best checkpoint does not change the ranking
(best-of: single $0.0076$, multi $0.0077$, xchg $0.0081$). The
exchange therefore neither wins nor loses on training-protocol
choices; its one significant, reproducible effect is rescuing
multi-scale degradation on the monotonic CALCE data.
