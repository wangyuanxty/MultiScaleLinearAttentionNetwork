...
\end{tabular}%\par\medskip{\footnotesize Baselines self-run with the official repositories (10 repeats per SP, seeds 1--10); published values are given in the source citations.}
\end{table}
...
8\,KB state per layer and no dynamic memory allocation. The
deployment advantage is deliberately \emph{not} parameter
minimization---the model is not the smallest among the compared
baselines---but deterministic memory behavior (a state that does not
grow with sequence length) and incremental inference that can absorb
hundreds of cycles of history. Full
