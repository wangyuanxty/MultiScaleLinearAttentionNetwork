...
Neither latency nor energy is reported: as
Section~\ref{sec:latency} explains, the emulator's cycle counter does not
track guest work faithfully enough to support one, and no on-device power
figure is available. All validation is on public laboratory cycling data;
field conditions such as partial cycling, pack-level effects, and
non-stationary loads remain open. Both the single-branch encoder and the
full multi-scale model are implemented in C and verified on emulated
hardware; the deployed footprints, the cross-architecture agreement and
the caveats that come with emulation are given in
