...
\midrule
\multicolumn{7}{l}{\emph{Average across all SPs and seeds: AE = 3.4 cycles, R\textsuperscript{2} = 0.9947, MAE = 0.0054.}} \\
\bottomrule
\end{tabular}
\end{table}
...
baselines---but a memory budget decoupled from context length: the
per-layer state is a fixed $d_k\times d_v$ matrix and the per-cycle
cost is constant, so the window is a free design parameter that can be
lengthened to meet application requirements without re-planning SRAM,
whereas a quadratic attention buffer forces a resolution/memory
trade-off as the window grows. Full
...
dynamic allocation) updated over the window scan. (5) Fused
...
MAE improves $1.5$--$7.9\%$---and extension to 300 epochs adds
