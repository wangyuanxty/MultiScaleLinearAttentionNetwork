...
def fig_compare():
    ...
    Values = mean MAE over the three SPs of each dataset, taken from
    the current tab:lit tables (self-run baselines included).
    """
    data = {
        # Every value is a three-SP mean MAE in Ah, matching tab:lit_all.  The
        # baselines were always in Ah; the "Ours" entries used to be normalized,
        # which on a log axis overstated our GOTION margin by 6.4x and
        # understated MIT by 3.7x.  src/check_fig_compare_data.py re-checks
