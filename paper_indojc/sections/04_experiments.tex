...
example) are left to the final version. Neither latency nor energy is
reported: the emulator's cycle counter does not track guest work
faithfully enough to support one, and no on-device power figure is
available. All validation is on public laboratory cycling data; field
conditions such as partial cycling, pack-level effects, and
non-stationary loads remain open.
